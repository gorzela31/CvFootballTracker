...
# ==========================================================================
RUN_NAME = "pipeline_frcnn_bt_keypoints"
...
def run_pipeline(
    ...
    keypoints_weights: Path = KEYPOINTS_WEIGHTS,
    ...
    # ---- 3. Inicjalizacja kalibratora keypoints ----
    print(f"[3/6] Inicjalizacja KeypointsHomography")
    calibrator = KeypointsHomography(model_weights=str(keypoints_weights))
    ...
    print(f"      Detekcja: Faster R-CNN | Kalibracja: YOLO Keypoints | Tracking: ByteTrack")
    print()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Pipeline: Faster R-CNN + ByteTrack + YOLO Keypoints Homography"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--keypoints-weights", type=str, default=str(KEYPOINTS_WEIGHTS),
        help="Sciezka do wag modelu YOLOv8-pose (keypoints boiska)"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    run_pipeline(
        ...
        keypoints_weights=Path(args.keypoints_weights),
        conf=args.conf,
